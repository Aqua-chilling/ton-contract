...
function initAquaJetton_init_args(src: AquaJetton_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeRef(src.content);
...
const AquaJetton_types: ABIType[] = [
...
const AquaJetton_getters: ABIGetter[] = [
...
const AquaJetton_receivers: ABIReceiver[] = [
...
export class AquaJetton implements Contract {
    
    static async init(owner: Address, content: Cell, max_supply: bigint) {
        return await AquaJetton_init(owner, content, max_supply);
    }
    
    static async fromInit(owner: Address, content: Cell, max_supply: bigint) {
        const init = await AquaJetton_init(owner, content, max_supply);
        const address = contractAddress(0, init);
        return new AquaJetton(address, init);
    }
    
    static fromAddress(address: Address) {
        return new AquaJetton(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  AquaJetton_types,
        getters: AquaJetton_getters,
        receivers: AquaJetton_receivers,
        errors: AquaJetton_errors,
    };
